	...
	function cs_event_meta_data($post) {
		...
		if ( $cs_event_meta <> "" ) {
			$cs_xmlObject = new SimpleXMLElement($cs_event_meta);
				...
				$event_sub_title = $cs_xmlObject->event_sub_title;
				$event_related_events = $cs_xmlObject->event_related_events;
				$event_related_events_title = $cs_xmlObject->event_related_events_title;
				$event_performers_title = $cs_xmlObject->event_performers_title;
				$event_ticket_options = $cs_xmlObject->event_ticket_options;
				$event_rating = $cs_xmlObject->event_rating;
				if(isset($cs_xmlObject->var_cp_trainer) && $cs_xmlObject->var_cp_trainer <> ''){
					$var_cp_trainer = $cs_xmlObject->var_cp_trainer;
					if ($var_cp_trainer)
					{
						$var_cp_trainer = explode(",", $var_cp_trainer);
	
					}
				} else {
					$var_cp_trainer = array();
				}
		}
		else {
            ...
            <div class="option-sec" style="margin-bottom:0;">
                <div class="opt-conts">
                	<ul class="form-elements">
						...
						<li class="to-field"><input type="text" name="event_sub_title" value="<?php echo $event_sub_title ?>" /></li>
                   </ul>
                    <ul class="form-elements">
                    ...
                    <ul class="form-elements">
                        <li class="to-label"><label>Related Events</label></li>
                        <li class="to-field">
                        	<div class="on-off"><input type="checkbox" name="event_related_events" value="on" class="myClass" <?php if($event_related_events=='on')echo "checked"?> /></div>
                            <p>Enable/Disbale Related Events</p>
                        </li>
                    </ul>
                    <ul class="form-elements">
                        <li class="to-label">
                            <label>Related Events Title</label>
                        </li>
						<li class="to-field"><input type="text" name="event_related_events_title" value="<?php echo $event_related_events_title ?>" /></li>
                   </ul>
                    <ul class="form-elements">
                        <li class="to-label">
                            <label>Performers Title</label>
                        </li>
						<li class="to-field"><input type="text" name="event_performers_title" value="<?php echo $event_performers_title ?>" /></li>
                   </ul>
                    <ul class="form-elements">
                        <li class="to-label"><label>Performers</label></li>
                        <li class="to-field">
                            <select name="var_cp_trainer[]"  multiple="multiple"  class="dropdown" style="height: 100px !important;">
                                <option value="0">Select Performer</option>
                                <?php
                                    query_posts( array('posts_per_page' => "-1", 'post_status' => 'publish', 'post_type' => 'teams') );
                                        while ( have_posts()) : the_post();
                                        ?>
                                            <option <?php if (in_array(get_the_ID(), $var_cp_trainer)) { echo 'selected="selected"';}?> value="<?php the_ID()?>"><?php the_title()?></option>
                                        <?php
                                        endwhile;
                                ?>
                            </select>
                   ...
                   <ul class="form-elements">
                    ...
                    <ul class="form-elements">
                        ...
                        <li class="to-field">
                        	<select name="event_rating" class="dropdown">
                            	<option value="0">--Select Rating--</option>
                                <?php
									for($i=1; $i<=10; $i++){
										?>
	                                        <option <?php if($event_rating==$i)echo "selected"?> value="<?php echo $i;?>"><?php echo $i;?></option>
                                        <?php
									}
                                ?>
                            </select>
